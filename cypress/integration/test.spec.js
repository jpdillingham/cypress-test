describe("top-level suite in suite file", () => {
  it("captures teams from config", { tags: ["#smoke", "#tag", "@team1", "@team2"] }, () => {
    // expected teams: @team1,@team2
    // tags: #smoke,#tag
    expect(true).to.equal(false);
  });

  describe("what", { tags: ["@suite_team"] }, () => {
    describe("about", () => {
      describe("nested suites?", () => {
        ...
        it("what happens to suite teams if a test config also has teams?", { tags: ["@team3", "@team4"] }, () => {
          // expected teams: @team3,@team4
          expect(true).to.equal(false);
        });

        it("and what about tests that define multiple teams in a string?", { tags: "#foo, #bar, @teamA, @teamB" }, () => {
